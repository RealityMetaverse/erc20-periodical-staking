...
///         pool can be short from the start; that case is the last test below.)
contract SolvencyPoC is Test {
    ...
    uint256 internal constant APY = 20;
    ...
    function _openBothDeposits(uint256 periodicalAmount, uint256 indefiniteAmount) internal returns (uint256 reserved) {
        ...
        vm.prank(periodicalStaker);
        staking.safeStake(0, PERIOD, periodicalAmount, APY);
        ...
        vm.prank(indefiniteStaker);
        staking.safeStake(0, 0, indefiniteAmount, APY);
    }
    ...
    function test_poolShortFromTheStart_periodicalClaimWaitsForTopUp() external {
        ...
        vm.prank(periodicalStaker);
        staking.safeStake(0, PERIOD, principal, APY);
        vm.prank(indefiniteStaker);
        staking.safeStake(0, 0, principal, APY);
