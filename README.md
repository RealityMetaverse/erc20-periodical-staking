...
v0.5.0 changes what the voucher's extra limit MEANS, adds an on-chain wallet block, bounds voucher lifetime, adds a voucher epoch, and publishes a version string. The voucher struct changes shape, so **every v0.4.0 voucher and signer is incompatible** - see the typehash below. The pre-release audit changed the struct once more (two new signed fields, `issuedAt` and `epoch`); see "Audit fixes" at the end of this section.
...
**The voucher gains three fields and one is renamed.** `extraLimit` is gone; `extraLimitPerCell`, `issuedAt` and `epoch` are new. The EIP-712 type is now (10 fields):
...
`keccak256` of that string is `0x3fa11eb88bbb17d4905cdc2219076bae492deebb6042a24c6a050bae86d38597`, which is the value compiled into `VOUCHER_TYPEHASH()`. (The 8-field type without `issuedAt` / `epoch`, hash `0xa7048f90...08bd49b`, was the pre-audit v0.5.0 draft and was never deployed.) The domain is unchanged: `{ name: "ERC20PeriodicalStaking", version: "1", chainId, verifyingContract }` - the domain `version` is **not** a contract version and stays `"1"`. Field order is the encoding order; a tuple built in the old order produces a valid signature over the wrong digest and every stake reverts `InvalidVoucherSignature`. Check `VERSION()` before blaming the key.
...
- `issuedAt` is the unix time the backend signed the voucher. It must not be in the future (`VoucherNotYetValid(issuedAt, now)`), and the signed lifetime `validUntil - issuedAt` must not exceed `maxVoucherValidity()` (`VoucherValidityTooLong(validUntil, issuedAt + maxVoucherValidity)`). Sign with the chain's clock in mind: an `issuedAt` a few seconds ahead of the block timestamp reverts until the chain catches up, so the backend should back-date `issuedAt` by a small skew allowance (and keep `validUntil - issuedAt` inside the ceiling).
- `epoch` must equal `voucherEpoch()` (`VoucherEpochMismatch(voucherEpoch, currentEpoch)`). It starts at `0`. The backend reads `voucherEpoch()` and signs the current value; after the owner calls `bumpVoucherEpoch()` every voucher signed for the old epoch is void.
...
**Wallet block.** `setWalletBlocked(address, bool)` and `setWalletsBlocked(address[], bool)` (admins and owner) bar a wallet from opening NEW stakes; `walletBlocked(address)` reads the flag and `stakeWithVoucher` reverts `WalletBlocked`. It is checked first, before the nonce is consumed. **It gates staking only** - withdraw, claim, `claimAll`, `claimRange` and existing deposits are untouched, and a block never traps funds. It stops ONE KNOWN wallet on-chain, even when a voucher for it was issued outside the backend's blocklist. It is **not** a defence against a leaked voucher signing key - the key holder signs for a fresh wallet. The levers for a key compromise are `closeStaking()` (any admin, immediately), then `bumpVoucherEpoch()` and `setVoucherSigner(newKey)` (owner).
...
**Voucher validity ceiling.** `maxVoucherValidity()` (`uint32` seconds) bounds the voucher's signed lifetime, `validUntil - issuedAt` (`VoucherValidityTooLong`), and `issuedAt` may not be in the future (`VoucherNotYetValid`). Before the audit the bound was measured from now, so a voucher signed "good for a year" reverted today but became usable in the last `maxVoucherValidity` seconds before its `validUntil`; with both ends signed it is never usable. This bounds vouchers the backend signed honestly - it does not contain a leaked key, which can always sign a fresh short-lived voucher (see `bumpVoucherEpoch` / `setVoucherSigner` / `closeStaking`). `setMaxVoucherValidity(uint256)` rejects `0` (`ZeroAmountProvided`) - the protection has no off switch, lower it instead. The constructor defaults it to `1800`, the maximum the backend's own `validity_seconds` can be set to, so a contract deployed outside the script is still usable. The backend reads this ceiling and refuses to sign with a 503 rather than issuing a voucher that would revert.
...
### Audit fixes (pre-release, still v0.5.0)

What the backend, ops and indexers must change. Finding numbers refer to the v0.5.0 audit; every one has a regression test under `test/audit-poc/`.

- **#8 Voucher: two new SIGNED fields, `issuedAt` and `epoch`.** New 10-field type and typehash above; field order is `... extraLimitPerCell, issuedAt, validUntil, epoch, nonce`. New storage `voucherEpoch()` (`uint64`, starts at 0; packed beside `maxExtraLimitPerCell` and `maxVoucherValidity` so the stake path reads it from a slot it already loads), new owner function `bumpVoucherEpoch()` (emits `UpdateVoucherEpoch(newEpoch)`), new errors `VoucherNotYetValid(issuedAt, currentTime)` and `VoucherEpochMismatch(voucherEpoch, currentEpoch)`. `VoucherValidityTooLong`'s second argument is now `issuedAt + maxVoucherValidity`. Check order in `stakeWithVoucher`: staking open -> wallet block -> signer set -> limit controller set -> wallet -> expiry -> issuedAt not in the future -> lifetime -> epoch -> nonce -> extra caps -> signature -> ... (unchanged after that).
- **#39 The voucher signer must be an EOA.** The signature is checked with plain ECDSA recovery (`ECDSA.tryRecover`); ERC-1271 contract signers are no longer supported and any malformed signature reverts `InvalidVoucherSignature`. This supersedes the v0.4.0 note about `SignatureChecker`.
- **#9 `closeStaking()` - admins and owner.** Sets staking closed and emits the same `UpdateActionAvailability(STAKING, false)` as the owner path. It can only close, and only staking; reopening stays owner-only via `changeActionAvailability`.
- **#10 `unfreezeDeposit` / `unfreezeDeposits` are now OWNER-ONLY** (were admins). Freezing stays admins-and-owner. An admin could otherwise unfreeze just ahead of the owner's seize.
- **#16 `setLimitController` checks the controller.** A non-zero address must answer `stakingContract()` with the staking contract's address, else `LimitControllerMismatch(address)`; an EOA or a contract without that function reverts without data. `ILimitController` gains `stakingContract()`. Deploy order therefore matters: deploy the controller for this staking contract first, then install it.
- **#18 The active-deposit cursor is a lower-bound hint.** One close advances `stakerActiveDepositStartIndex` by at most 256 closed deposits and stores the progress, so closing a deposit costs at most about 818k gas however many closed deposits sit behind it (it was unbounded). Everything before the cursor is closed; deposits at or after it may be closed too, and the cursor can sit below the deposit count when all are closed. `claimAll` and `checkClaimableDataFor` check every deposit's status themselves, so nothing else changes. Off-chain code must not treat the cursor as exact.
- **#19 Bounds.** Base APY `<= 1_000_000` bps in `pushStakingPhase` / `addStakingPeriod` / `setPhasePeriodData`, period `<= 36_500` days in `addStakingPeriod`, `setMaxExtraApyBps <= 1_000_000`; all revert `ValueTooHigh(provided, max)`. On the lens, `getRewardRequiredForTargets` and `getRewardPoolShortfall` saturate at `type(uint256).max` instead of reverting when a target is "unlimited".
- **#33 Events.** The constructor emits `TransferOwnership(address(0), deployer)`. `TransferOwnership(address indexed from, address indexed to)` now has both parameters **indexed** - same topic0, but the addresses moved from data to topics, so re-generate the ABI an indexer decodes with. `popStakingPhase` emits `ChangeStakingPhase(previous)` after `RemoveStakingPhase` when it rolls the current phase back. `UpdateActionAvailability(Types.DataType action, bool isOpen)` is now `UpdateActionAvailability(Types.DataType indexed action, bool isOpen)` — same topic0, but `action` moved from the data field to topic1, so an indexer that decodes it from data must be re-generated.
- **#34** Popping the phase that is current re-opens the previous phase for staking. Unchanged, now documented, and announced by the event above.
- **#36 `setTreasury` rejects the staking contract itself** (`InvalidTreasury`). A treasury the token refuses to pay (blacklist) makes seizes revert until the treasury is changed; the deposits stay frozen meanwhile.
- **#37** Empty `freezeDeposits` / `unfreezeDeposits` / `seizeDeposits` / `setWalletsBlocked` batches revert `EmptyBatch`. A seize is booked under the wallet's and the total `WITHDRAWAL` counters although the treasury received the principal: indexers must key on `SeizeDeposit` / status `SEIZED`. Send freezes through a private relay - a freeze seen in the public mempool can be front-run by a withdrawal.
- **#17, #23, #38 (documentation only).** `withdrawDepositPartial`'s `minReward` is a floor for a REDUCED payout only (it is not consulted when the pool covers the accrued reward), and an accepted reduced payout forfeits the remainder. Raising a wallet's controller limit does not re-attribute bonus already charged to an open deposit; it frees when that deposit closes. The misspelled `setMiniumumDeposit` selector is kept on purpose for ABI compatibility with v0.2.4 tooling.

**LimitController: `stakingContract` is immutable.** It is fixed by the constructor (`constructor(address _stakingContract)`, unchanged) and `setStakingContract(address)` has been **removed from the ABI**; `StakingContractSet` now fires once, from the constructor, and never again. Re-pointing a controller at a different staking contract means deploying a new controller. `setLegacyStakingContract` is unaffected.

New functions: `closeStaking`, `bumpVoucherEpoch`, `voucherEpoch`, `advanceCursor`; `ILimitController.stakingContract`.

**#18 `advanceCursor(address wallet, uint256 maxSteps) returns (uint256 newStartIndex)` — permissionless.** Maintenance only: it walks a wallet's active-deposit cursor past deposits that are already closed and can never move it past an open one, so it changes no balance, no counter and no deposit, and a call that finds nothing to skip writes nothing. The close paths run that scan only as a side effect of a claim, withdrawal or seize, so once every deposit of a wallet is closed nothing triggers it again and a cursor left behind by the `MAX_CURSOR_SCAN` (256) cap would stay there forever, making every later `claimAll` and `checkClaimableDataFor` re-walk the closed tail. `maxSteps` is clamped on both sides: `0` **or any value above** `MAX_CURSOR_ADVANCE` (1024) means 1024, and values in between are honoured exactly — so passing `type(uint256).max` does not catch a wallet up in one shot, it does 1024 steps and must be called again. It **emits no event** (deliberate, for the byte budget), so confirm progress from the return value or `stakerActiveDepositStartIndex(wallet)` rather than from logs; comparing against a pre-call read distinguishes “moved” from “already current”. It is also deliberately **ungated** — no `nonReentrant`, no availability modifier, no owner/admin check — so a cursor stays payable-down even after ops closes staking, withdrawal or claim. That is safe because it makes no external call and writes one storage slot; do not “tidy it up” by adding a modifier. **Ops:** worth a bot that calls it for wallets whose deposits are all closed — anyone can call it, including the wallet itself or an indexer. The cursor remains a lower-bound hint in every case: readers check each deposit's status themselves, so a lagging cursor costs gas, never correctness.
New events: `UpdateVoucherEpoch(uint256 newEpoch)`.
New errors: `VoucherNotYetValid`, `VoucherEpochMismatch`, `LimitControllerMismatch`, `ValueTooHigh`, `InvalidTreasury`, `EmptyBatch`.

## Deploying v0.5.0 with DeployV050.s.sol

`script/DeployV050.s.sol` creates a like-for-like copy of **the same network's** v0.2.4 contract: it reads the program from chain, deploys `ERC20PeriodicalStaking(sourceToken)` and `LimitController(staking)`, applies the copy, then re-reads every value from the new contracts and `require`s it equals the source.
...
Required: `VOUCHER_SIGNER` (backend voucher signer), `TREASURY` (receiver of seized principal), `MAX_EXTRA_APY_BPS` (e.g. 500 = 5.00%), `MAX_EXTRA_LIMIT_TOTAL` (token wei). Optional: `NEW_OWNER` (**both** contracts are two-step: the deploy only nominates, and `NEW_OWNER` must `acceptOwnership()` on the staking contract *and* on the LimitController; the deployer owns both until then), `ADMINS` (comma-separated), `OPEN_STAKING` (default false), `REWARD_TOP_UP` (default 0 = no funding; >0 = approve + `provideReward` from the deployer), `WALLET_LIMITS_FILE`, `SOURCE_STAKING` and `REQUIREMENT_CHECKER_V2` (default: the per-chain table above), `ALLOW_SHARED_ROLES` (default false; the preflight fails when one address holds two roles — a voucher signer that is also the deployer, `NEW_OWNER`, the treasury or an admin; a treasury that is the token, a staking contract, `NEW_OWNER`, or an address in `ADMINS` (an admin can freeze and seize, and the treasury receives what is seized, so one key would both take the funds and get them) — and `true` downgrades each clash to a warning, for testnets. Duplicate entries in `ADMINS` are always a warning only, never fatal).
...
Without `--broadcast` the run is a simulation. With `--broadcast` it signs with the Foundry keystore account (`--account "$DEPLOYER_ACCOUNT" --sender "$DEPLOYER_ADDRESS"`, created with `cast wallet import <name> --interactive`), and adds `--verify` when `ETHERSCAN_API_KEY` (or `POLYGONSCAN_API_KEY`) is set. A `PRIVATE_KEY` in the env file is accepted as a fallback but never printed or passed as a flag; with both set the keystore wins. Extra forge flags go in `FORGE_ARGS` (quoted); a flag named there is not added again by the wrapper, so `FORGE_ARGS` also overrides the robustness defaults below. `FORGE_ARGS` is an **allowlist**, not a denylist: only `-v`/`-vv`/`-vvv`/`-vvvv`/`-vvvvv`, `--slow`, `--isolate`, `--legacy`, `--force`, `--no-cache`, and the value flags `--gas-estimate-multiplier`, `--gas-price`, `--priority-gas-price`, `--gas-limit`, `--rpc-timeout`, `--timeout`, `--retries`, `--delay`, `--out`, `--cache-path`, `--optimizer-runs`, `--evm-version` (as `--flag value` or `--flag=value`) may appear. Anything else — a bare word, anything after `--`, an unknown flag — is rejected by name before the build runs. A denylist was tried first and leaked: `--resume` re-submits a cached broadcast, so it **sends** while the banner still says “simulation”; `--skip-simulation`, `--private-keys`, `--accounts`, `--interactives` and `-i` all slipped past it too. Signing, network and verification are chosen by `DEPLOYER_ACCOUNT`/`PRIVATE_KEY`, `RPC_URL` and `ETHERSCAN_API_KEY` in the env file, never by `FORGE_ARGS`.
...
Limits worth knowing: a resume must run **before** `NEW_OWNER` calls `acceptOwnership()` on either contract, because it still needs owner rights on both (the preflight refuses a `RESUME_STAKING` or `RESUME_CONTROLLER` the deployer no longer owns); if a phase was pushed while periods were missing, the phase's cells for those periods are wrong, the self-check fails loudly and the right answer is a fresh deployment, not a resume.
...
6. `NEW_OWNER`: `acceptOwnership()` on the staking contract **and** `acceptOwnership()` on the LimitController. Both are `Ownable2Step`, so the deploy only nominated; until both calls land the deployer still owns both contracts and the hand-off is not done.
7. When ready: `changeActionAvailability(STAKING, true)` on the new contract.

After a broadcast, re-run the configuration self-check against what was actually mined:

```
./script/deploy-v050.sh <network> --verify-only
```

It sends nothing (no `--broadcast`) and re-reads every copied value from the deployed contracts, so it catches a transaction that was dropped or reverted after the simulation had already "passed". This is not optional: the self-check inside a broadcast run reads the *simulation's* state, not the chain's. It needs `VERIFY_STAKING`, `VERIFY_CONTROLLER` and `DEPLOYER_ADDRESS` in the env file, and the rest of that file must still hold the values the deployment was broadcast with. The ownership check accepts both stages — deployer still owning with `NEW_OWNER` pending, and `NEW_OWNER` having accepted on both contracts — so it is equally valid before and after step 6, but the two contracts must be at the **same** stage: a half-finished hand-off (`acceptOwnership()` on one, forgotten on the other) fails with `check: half-finished hand-off` and prints which contract is still pending. Two things it deliberately does not treat alike:

- **action availability is informational.** `STAKING`/`WITHDRAWAL`/`CLAIM` availability is admin-mutable, and step 7 of the hand-off is `changeActionAvailability(STAKING, true)`, so comparing it against `OPEN_STAKING` for ever would make `--verify-only` fail on every healthy deployment. A mismatch is logged as `NOTE: ... informational only`. It is still a hard failure in the self-check that runs at deploy time, when nothing has touched the contract yet.
- **the reward pool is compared properly.** With `REWARD_TOP_UP` > 0 the check is `rewardPool() >= REWARD_TOP_UP`, not merely non-zero; a shortfall prints pool, expected and shortfall, then fails. In `--verify-only`, `REWARD_TOP_UP` is an *expectation about the chain*, so once rewards have been paid out and the pool has legitimately dropped, set `REWARD_TOP_UP=0` in the env file for verification runs.

Everything else — token, voucher signer, treasury, caps, limits, phases, periods, APYs, wallet limits, admins — is compared strictly in both modes.
...
### Current v0.5.0 tests

Tests: `test/v050/` covers vouchers (`Voucher.t.sol`), basis-point APYs and caps (`ApyBps.t.sol`), the LimitController with a legacy contract including the real vendored v0.2.4 contract (`LimitControllerLegacy.t.sol`), freeze and seize (`FreezeSeize.t.sol`) and gas budgets (`GasBudget.t.sol`); the existing suites are migrated to voucher staking and the invariant handler freezes and seizes. `RequirementCheckerV2Integration.t.sol` now uses the vendored v0.2.4 contract as its checker consumer, since v0.4.0 has no hook. `test/v050/DeployV050Local.t.sol` drives the deploy script's `deployFrom()` against a locally deployed vendored v0.2.4 source, with no fork and no RPC, so the deployment path is covered by the default `forge test`: the preflight's APY/period/cap bounds and role-distinctness checks, the two-step ownership hand-off on both contracts, `REWARD_TOP_UP` and its resume guard, and the idempotence of a resumed run (asserted on the contracts' own events -- a resume of a finished deployment emits nothing).

Default profile (`forge test`, which excludes the invariant campaigns via `no_match_path`): 60 suites, 1033 passed, 0 failed, 5 skipped. The 5 skips are the two opt-in fork suites (3 + 2), which skip without an RPC; they are the repo's only skip mechanism, an `onlyFork` modifier reached through a raw cheat-code interface rather than `vm.skip`. The count includes the regression and adversarial suites under `test/audit-poc/`, added while fixing the audit findings above. The full profile (`FOUNDRY_PROFILE=full forge test`, ~45 min) has not been re-run since those suites were added, so no count is quoted for it here.
...
These notes describe v0.4.0 as recorded in Git history. The v0.5.0 audit changes above do not change earlier versions or their deployments.
...
**Historical v0.4.0 tests (commit `8f5812e`):** `test/v040/` covers vouchers (`Voucher.t.sol`), basis-point APYs and caps (`ApyBps.t.sol`), the LimitController with a legacy contract including the real vendored v0.2.4 contract (`LimitControllerLegacy.t.sol`), freeze and seize (`FreezeSeize.t.sol`) and gas budgets (`GasBudget.t.sol`); the existing suites are migrated to voucher staking and the invariant handler freezes and seizes. `RequirementCheckerV2Integration.t.sol` now uses the vendored v0.2.4 contract as its checker consumer, since v0.4.0 has no hook. Full suite: 688 passed, 0 failed, 1 skipped (the opt-in `LegacyStakingInvariants`).
...
The contract itself builds in under a minute. The test suite is the slow part: compiling all 95 test files takes
about 29 minutes (one `solc` process, `via_ir`) and the 38 invariant campaigns run for about 13 more. So there are
two ways to run it:
